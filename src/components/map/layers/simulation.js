...
const LAYER_NAME = 'SIMULATAION_LAYER'

export const simulationLayer = (subProps , mapId)=>{
	let { 
		visible = true,
		data = [],
		opacity = 1,
	...
	// data.map((group)=>{
	// 	let { polygon, support } = group

	// 	_points = randomPointsInPolygon( polygon , support )
	// 	console.log( _points )
	// 	// bbox = turf.bbox( turf.lineString(polygon) )
	// 	// _points = turf.randomPoint( support , { bbox } )
	// 	// // console.log(_points['features'])
	// 	// _points = _points['features']
	// 	// _points = _points.map((geojson)=> geojson['geometry']['coordinates'])
...
export function randomPointsInPolygon( polygon, number ){
	let triangles = triangulate(polygon)

	let randomTriangle, randomPoint
	let points = []
	for(let i = 0;i < number;i++){
		randomTriangle = selectRandomTriangle(triangles);
		randomPoint = calcRandomPoint(randomTriangle);
